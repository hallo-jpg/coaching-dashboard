# Langfristiger Entwicklungsplan – Stefan

*Erstellt: April 2026 · Überarbeitet: 21. September 2026 (Wintersaison-Ausrichtung)*
*Kein Ablaufdatum – wird bei wesentlichen Veränderungen aktualisiert*

---

## Übergeordnetes Entwicklungsziel

**Zwei Motoren parallel entwickeln: FTP auf dem Rad und Laufleistung.** Rennergebnisse sind Indikatoren, kein Endziel.

Bis September 2026 war Laufen die zweite Sportart, die nebenher lief. Ab Winter 2026/27 ist es ein gleichrangiges Entwicklungsziel mit eigenem Volumen, eigener Progression und eigenen Tests — weil Stefan das so will und weil die Datenlage zeigt, dass dort der größte ungenutzte Hebel liegt (2026: ~250 Laufkilometer im ganzen Jahr).

**Messgrößen:**
- **Rad:** FTP nach formalem Test (3+10min, outdoor, XCadey). Keine spekulativen Anpassungen.
- **Lauf:** 10km-Wettkampfzeit + monatlicher HF-160-Standardcheck (Pace bei HF 160).

---

## Mehrjährige FTP-Entwicklung

⚠️ **Baseline-Reset September 2026:** Die 305W stammen vom 4iiii am alten Rad (Test 4.4.2026). Seit dem Setup-Wechsel (Tarmac SL8 / Cybrei / XCadey) ist die Messquelle eine andere. Der KW40-Test setzt den neuen Nullpunkt — alle Zielwerte unten beziehen sich ab dann auf diese Baseline.

| Zeitpunkt | FTP-Ziel | Anmerkung |
|---|---|---|
| Frühjahr 2026 | 305W (4iiii) | historisch, nicht mehr vergleichbar |
| **KW40 2026** | **Baseline neu** | erster Test am XCadey · misst Form bei CTL 23, nicht Potenzial |
| März 2027 | **+8–12% über Baseline** | nach dem Winterblock |
| Herbst 2027 | +12–18% über Baseline | nach zweitem Aufbaujahr |
| Langfristig | ~4,0 W/kg | Plateau für Stefans Profil (~5–7 Jahre) |

**Wachstumserwartung**: +3–5% FTP pro Saison bei konsistenter Ausführung. 2026 lag darunter — nicht wegen des Trainings, sondern wegen zweier Infektpausen und des Unfalls (CTL 48 → 16).

---

## Laufen – Mehrjährige Entwicklung

| Zeitpunkt | Ziel | Volumen |
|---|---|---|
| 2026 ✅ | Erster 10km-Wettkampf: **1:05:08 / 6:31/km** (20.9., PR −6:20) | ~6,5 km/Woche |
| **Winter 26/27** | **10km sub-60 (5:59/km)** · Kadenz 165–170 stabil · 5km <28:30 | **28–32 km/Woche** |
| 2027 | 10km <57:00 · erster Halbmarathon optional | 32–38 km/Woche |
| Langfristig | Triathlon-Option (Olympic) oder eigenständige Laufrennen | 35–40 km/Woche |

**Ursachenreihenfolge für den Rückstand** (Analyse 20.9.2026, gilt weiter):
1. **Laufvolumen / Durability** — mit Abstand der größte Faktor
2. **Körpergewicht** — beim Laufen wird jedes Kilo bei jedem Schritt getragen (~200 ml O₂/kg/km); grob 4–5 kg ≈ 2–3 min auf 10 km. Stefans eigene Einordnung (21.9.2026): aktuelles Gewicht in Ordnung, **ggf. Richtung 90 kg, darunter nicht** — bei 190 cm eine gute Basis. 93,6 → 90 kg wären grob 1–1,5 min auf 10 km. Unterstützender Faktor, **kein Coach-Ziel, kein Ernährungsprogramm**.
3. **Ökonomie / Kadenz** — 3–6%

Nie als Talentfrage rahmen. Nie Ökonomie als „das Problem" bestätigen.

---

## Laufbelastungs-Budget (Orthopädie-Regeln)

*Grundlage: Wadenproblematik seit 8/2026 (MTSS-Verdacht Soleus-Tibia-Übergang) und Stefans eigene Beobachtung, dass langsames Laufen ihn mehr kostet als schnelles. Die Beobachtung ist richtig — hier die vollständige Begründung und die Konsequenzen.*

**Warum langsames Laufen bei ihm teuer ist:**
Die Spitzenkraft pro Schritt ist bei langsamem Tempo niedriger — das ist aber nur eine von drei Lastdimensionen. Bodenkontaktzeit und Zeit-unter-Spannung des Soleus steigen beim langsamen Laufen. Knochenbelastung akkumuliert nicht linear mit der Dehnung, sondern hoch potenziert × Zyklenzahl. Bei ~140 spm und 94 kg heißt langsam: Fuß weit vor dem Schwerpunkt, maximaler Bremsimpuls, maximales Biegemoment auf die Tibia. **Langsames Trotten bei niedriger Kadenz ist sein teuerster Laufmodus.** Deckt sich mit dem Befund: die Wade tat nur beim langsamen Laufen weh.

**Warum die Lösung trotzdem nicht „mehr schnell statt langsam" ist:**
Tibia- und Achillessehnenbelastung skalieren mit dem Tempo (Achillessehnenkräfte im Tempobereich ~7–8× Körpergewicht). Easy-Volumen durch Tempo zu ersetzen kauft bei niedriger Laufbasis eine andere Überlastung ein.

**Die Regeln, die daraus folgen:**

| Regel | Begründung |
|---|---|
| **Kein langsames Trotten.** Easy = 7:15–7:45/km @ HF 155–165 | briskes Ende des Bandes, kürzere Kontaktzeit |
| **Kadenz in jeder Easy-Einheit, Metronom.** Ziel 165–170 | erweitert das orthopädische Budget — wichtigster Einzelhebel |
| **Aerobes Volumen kommt vom Rad** | null Impact, und es dient ohnehin dem FTP-Ziel |
| **Jeder Lauf hat einen Zweck** — Qualität, Longrun, Kadenz-Easy. Keine Füllkilometer | Volumen ohne Zweck kostet Gewebe ohne Ertrag |
| **2×/Woche Lauf-Athletik, 15min** (Soleus-Wadenheben gebeugtes Knie, einbeinige RDL, Hüfte seitlich) | bei 94 kg nicht optional |
| **+10%-Regeln**: Wochenkilometer max. +10%/Woche, längster Lauf max. +10% gegenüber den letzten 30 Tagen | einzige belastbare Volumen-Leitplanke |
| **Schuhrotation**, gedämpfte Trainingsschuhe bei 94 kg, Kilometerstand im Blick | Materialermüdung ist bei hohem Körpergewicht früher relevant |

**Prinzip**: Laufvolumen wächst langsamer als Radvolumen. Verletzungsprävention hat Vorrang vor Tempo-Entwicklung — nicht weil Tempo unwichtig wäre, sondern weil jede Zwangspause mehr kostet als jede einzelne Einheit bringt.

---

## Krankheitsprävention (Winterprotokoll)

*2026 gingen rund 4 Wochen durch Infekte verloren (KW16 April, ~10.–22. August) — beide mitten im Aufbau. Das ist der teuerste einzelne Posten der Saison, teurer als jede verpasste Einheit.*

**Strukturell (wirkt am stärksten, kostet nichts):**
- **3:1-Rhythmus mit echter Entlastungswoche** (−40–50% TSS). Nicht verhandelbar, auch nicht wenn es gerade gut läuft.
- **HRV-Gate** statt Bauchgefühl: HRV ≥45 & Ruhepuls ≤58 → grün · 38–44 → gekürzt · <38 oder RP >60 → Ruhetag. Qualität wird verschoben, nie gestrichen.
- **Keine harte Einheit in den 48h nach Flug/Reise** oder nach einer Nacht <6h Schlaf. Beide Ausfälle 2026 hatten Reise im Umfeld.
- **Kohlenhydrate während Einheiten >75min** (40–60 g/h). Nüchtern-Hart drückt die Immunfunktion messbar — Fueling ist auch Infektprophylaxe.
- **Alkohol vor Schlüsselwochen meiden.** Referenzfall 1./2.8.2026: HRV −20, Ruhepuls +9, zwei Trainingstage verloren.

**Medizinisch (Größenordnungen, ersetzt keine ärztliche Beratung):**
- **Vitamin D**: in München Oktober–März praktisch keine Eigensynthese; niedriger Spiegel ist einer der wenigen gut belegten Zusammenhänge mit Atemwegsinfekten bei Sportlern. **Wert messen lassen (25-OH-D)**, dann gezielt supplementieren statt blind. *(Stand 21.9.: Wert unbekannt, offen.)*
- **Zinkacetat-Lutschtabletten** innerhalb der ersten 24h nach den ersten Symptomen verkürzen Erkältungen moderat (~1–2 Tage). Danach wirkungslos → vorrätig halten.
- **Grippeimpfung** im Oktober, wenn keine Vorbehalte bestehen: eine Grippe kostet im Winterblock 2–3 Wochen.
- **Kein erkennbares Infekt-Muster** (Stefan, 21.9.2026) — keine vermeidbare Einzelquelle. Es wirken deshalb nur die strukturellen Maßnahmen oben, nicht Verhaltensänderungen an einer bestimmten Stelle.
- **Handhygiene / Gesicht nicht anfassen** — im Alltag der stärkste Einzelfaktor (Büro, ÖPNV).

**Wenn es passiert:** Neck-Check-Regel (Coach-Skill, Schritt K). Oberhalb Hals: symptomfrei +24h → LIT. Unterhalb Hals: +72h. Fieber: fieberfrei +24h, dann Neck-Check. **Der Block wird nicht nachgeholt** — deshalb bekommt jeder 3-Wochen-Block einen eigenen Abschluss.

---

## Leistungsmessung: zwei Radquellen getrennt halten

Seit September 2026 gibt es zwei Leistungsquellen, die **nicht** vergleichbar sind:

| Ort | Rad | Quelle |
|---|---|---|
| Outdoor | S-Works Tarmac SL8 | XCadey Spindle-Powermeter |
| Indoor | Canyon Aeroad (ohne PM) | Tacx Flux S (Rollenleistung) |

Direktantriebs-Rollen lesen typischerweise 2–5% unter Kurbel-/Spindelmessern (Antriebsstrangverluste), der Flux S hat ±3% Toleranz, dazu kommt die andere Sitzposition. Realistischer Gesamtversatz: 5–15%.

**Vorgehen (Stefans Entscheidung 21.9.2026) — bewusst pragmatisch:**
Der FTP-Test läuft **indoor auf der Rolle mit dem S-Works**. Das Rad wird für den Test einmalig
aufgespannt, der XCadey zeichnet parallel zur Tacx-Leistung auf. Ergebnis aus einer einzigen Einheit:

1. **FTP = XCadey-10min × 0,90** → der eine gültige FTP-Wert, für draußen wie drinnen.
2. **Offset-Faktor** = Tacx-Ø ÷ XCadey-Ø über dieselben 10min → daraus die Indoor-Zielwatt für die
   normalen Rolleneinheiten auf dem Aeroad (dort misst nur die Tacx).

**Ein FTP-Wert, ein Korrekturfaktor.** Keine zwei parallel gepflegten FTPs — dafür ist der Ertrag zu klein.

**Aufzeichnung am Testtag (Stefans Setup, 21.9.2026):**
**XCadey → COROS DURA** · **Rolle → Tacx-App**. Zwei Aufzeichnungen derselben Fahrt, daraus der
Vergleich der 10min-Mittelwerte. Die DURA ist die maßgebliche Datei (XCadey = Referenzmessgerät),
die Tacx-Aufzeichnung dient nur dem Faktor.
⚠️ **Doppelzählung vermeiden:** Laden beide Quellen hoch, liegt dieselbe Fahrt zweimal in intervals.icu
und CTL/ATL zählen doppelt. Eine der beiden löschen oder von der Belastungsrechnung ausnehmen —
behalten wird die DURA-Datei.
**Der Test läuft nicht im ERG-Modus** — ERG hält eine feste Vorgabe, ein Maximaltest ist darin nicht
möglich. Deshalb wird für Tests das **S-Works aufgespannt**: auf dem Indoor-Canyon ist die Schaltung
ausgebaut, ohne Gänge ist Level-Modus nicht fahrbar. Im normalen Training ist ERG dagegen der
Standardmodus — und für 2–6-Minuten-Intervalle ideal.
Level-/Sim-Modus oder freie Fahrt, Intensität kommt von den Beinen.

**Testbedingungen konstant halten** (sonst ist die nächste Messung nicht vergleichbar): immer indoor
auf der Rolle mit dem S-Works, Ventilator an, 20min Warmfahren, XCadey vor dem Start nullen, Tacx-Spindown
nach 10min Warmfahren. Ein Indoor-Test fällt typisch etwas niedriger aus als outdoor (Hitze, Motivation) —
solange alle Tests gleich laufen, ist das egal, weil nur die Differenz zählt.

**Dauerregel:** Rollen- und XCadey-Watt im Training nie gegeneinander vergleichen. Fortschritt immer
Quelle gegen sich selbst.

---

## Jahreszyklus-Modell (wiederholt sich jährlich)

Unabhängig von konkreten Rennterminen. Rennen werden in dieses Rahmenwerk eingepasst, nicht umgekehrt.

| Phase | Typische Dauer | Monate | Kerninhalt |
|---|---|---|---|
| **Winterbasis** | 14–18 Wochen | Nov–Feb | Maximales LIT-Volumen · KA-Erhalt · Laufvolumen aufbauen |
| **Frühjahrsaufbau** | 6–8 Wochen | Mär–Apr | SwSp · KA progressiv · CTL aufbauen · Lauf-Spezifik |
| **Rennsaison / HIT-Block** | 8–10 Wochen | Apr–Jun | HIT_EB · HIT_IE · LC · Rennen |
| **Sommererholung** | 3–4 Wochen | Jul | Aktive Erholung · unstrukturiert · CTL erholen |
| **Herbstaufbau** | 8–10 Wochen | Aug–Sep | Zweiter SwSp+KA-Block · FTP-Test |
| **Herbst-HIT** | 6–8 Wochen | Okt–Nov | Zweiter HIT-Block · optionale Herbstrennen |
| **Übergang** | 2–3 Wochen | Nov/Dez | Aktive Pause · Kraft · kein strukturiertes Intervalltraining |

**Wichtig**: Ein Modell, keine Vorschrift. 2026/27 weicht bewusst ab — der Übergang liegt über Weihnachten (KW52/53), weil er dort ohnehin stattfindet.

---

## CTL-Jahreskurve (Richtwerte)

| Phase | CTL-Zielbereich | Charakter |
|---|---|---|
| Tiefpunkt (Übergang) | 40–50 | bewusst niedrig – aktive Erholung |
| Winterbasis-Peak | 60–75 | maximale aerobe Kapazität aufbauen |
| Rennsaison-Peak | 65–80 | Intensität > Volumen |
| Sommertief | 50–60 | schnelle Erholung, nicht zu lang |
| Herbst-Peak | 70–82 | zweites CTL-Hoch, FTP-Test-Vorbereitung |

**Faustregel**: CTL-Rückgang > 15 Punkte in 2 Wochen → Überbelastung. CTL-Aufbau > 7 Punkte/Woche → zu schnell.
**Stand 21.9.2026:** CTL 23,3 nach Unfall- und Krankheitsjahr. Winterziel 60–68 bis Ende Januar = ~2,5 Punkte/Woche. Konservativ und haltbar.

---

## Trainingsvolumen-Entwicklung

| Zeitraum | Stunden/Woche | Anmerkung |
|---|---|---|
| Sept 2026 (Ist) | ~3,2h | nach Rennen und Verletzungsjahr |
| **Winter 26/27 (Ziel)** | **9–10h** | in 8–10 Wochen aufgebaut, nicht sofort |
| 2027 | 9–11h | Peak-Wochen 13–15h |
| 2028+ | 10–12h | Vollzeit-Amateur-Grenze |

**Rampe ist wichtiger als Zielwert.** Von 3 auf 9–10h ist eine Verdreifachung — max. +10–15% Volumen pro Woche, jede vierte Woche runter. Wer die Rampe überspringt, landet im Infekt (Muster 2026).

**Zeitrahmen (Stefans Vorgabe 21.9.2026):** werktags max. 90min (Ausnahme bis 2h), vor oder nach der Arbeit; Wochenende trägt die langen Einheiten. Konkrete Wochenplanung rollierend 1–2 Wochen im Voraus.

---

## Kraft & Athletik – Ganzjahres-Strategie

**Equipment (Stefan, 21.9.2026): Eigengewicht und Widerstandsbänder. Kein Kraftraum, keine schweren Hanteln.**
Für die Laufverletzungsprävention vollständig ausreichend — die wirksamen Übungen sind einbeinig und
brauchen kein Eisen. Für die FTP bringt Bändertraining wenig; der radspezifische Kraftreiz bleibt
**KA auf der Rolle bei 55 rpm**.

| Baustein | Umfang | Inhalt |
|---|---|---|
| **Lauf-Athletik** (Pflicht) | **2×15min/Woche**, an Laufeinheiten angehängt | Wadenheben gebeugtes Knie (Soleus), einbeinige RDL, Step-ups, einbeinige Hip Thrust, seitliche Hüfte mit Band, Copenhagen Plank |
| **Ganzkörper** (optional) | 1×25–30min/Woche, nach Rad oder Lauf | Liegestütze, Banded Row, Overhead Press, Rumpf — Erhalt, kein Aufbau |
| **KA auf der Rolle** | im Rad-Plan enthalten | 91% FTP bei 55 rpm — der eigentliche Kraftreiz fürs Rad |

**Progression ohne Zusatzgewicht:** Wiederholungen hoch (3×12 → 3×20) → langsamere Exzentrik (3s runter)
→ einbeinig → Rucksack als Zusatzlast. In dieser Reihenfolge, nicht gleichzeitig.

**Tapering:** Kraft ab T-10 weglassen. Lauf-Athletik darf bleiben (kein Ermüdungsbeitrag).

---

## FTP-Test-Fenster (Saisonal)

| Testfenster | KW | Datum | Rationale |
|---|---|---|---|
| ~~🔬 Frühjahrstest 2026~~ | ~~KW21~~ | — | gestrichen 03.05.2026 (Krankheitspause KW16) |
| 🔬 **Herbsttest 2026 (Baseline)** | **KW40** | 28.9.–4.10.2026 | erster Test am XCadey · Rolle + Gegenprobe · Nullpunkt für den Winterblock |
| 🔬 **Wintertest 2027** | **KW01** | 4.–10.1.2027 | nach 11 Wochen Aufbau · zusammen mit 5km-Zeitfahren |
| 🔬 **Abschlusstest Winter** | **KW09** | 1.–7.3.2027 | Bilanz Winterblock · zusammen mit 10km-Benchmark |

**Testprotokoll:** Sentiero 3+10min · **indoor auf der Rolle mit dem S-Works** · FTP = XCadey-10min-Avg × 0,90 · Bedingungen bei jedem Test identisch halten

**Coach-Verhalten:** 3 Wochen vor Testfenster automatischer Hinweis. Stefan bestätigt oder verschiebt um max. 1 KW. Kein Test im Tapering.

---

## Laufspezifische Tests

| Test | Rhythmus | Protokoll |
|---|---|---|
| **HF-160-Standardcheck** | monatlich | gleiche Strecke, morgens, gefrühstückt, ≥48h nach harter Einheit, 30min @ HF 160 → Pace |
| **5km-Zeitfahren** | alle ~8 Wochen | flach, gleiche Strecke |
| **10km-Benchmark** | 2×/Jahr | möglichst als Volkslauf |

Einzelne Einheiten mit Confoundern (nüchtern, nach Radbelastung, nach Pause) sind **keine** Fortschrittsmessung. Erst 3–4 vergleichbare Punkte werden bewertet.

---

## Wann den Plan anpassen?

Der Langfristplan wird **nicht** bei jeder Trainingsanpassung geändert. Er wird aktualisiert wenn:

- FTP-Messung wesentlich von der Prognose abweicht (± 10W oder mehr)
- Langfristige Ziele sich ändern
- Neues medizinisches Wissen oder Verletzungshistorie die Planung beeinflusst
- Stefan explizit einen Review anfordert

**Nicht** anpassen bei: Krankheit, ausgefallenen Wochen, kurzfristigen Planänderungen – das ist Sache des Saisonplans und der Wochenpläne.
